...
from listings.demo_assets import DEMO_IMAGES, ensure_demo_image_dirs, FRONT_DEMO
...
def main():
    ...
    # Главная настройка: по умолчанию не пишем демо-фото в Django MEDIA_ROOT (`media/`).
    # Если всё же нужно загрузить изображения, запусти так:
    #   LOAD_DEMO_IMAGES=1 python seed_demo_no_images.py
    load_demo_images = os.getenv("LOAD_DEMO_IMAGES", "0").strip().lower() in {
        "1",
        "true",
        "yes",
    }

    # Главное меню (блок "Фотографии агентства" на главной) берёт данные из API
    # `company-gallery/`. Чтобы статические картинки `/frontend/public/demo/gallery_*.jpg`
    # реально отображались, очищаем записи галереи в БД.
    CompanyGalleryImage.objects.all().delete()

    # Обновляем статические файлы `/frontend/public/demo/gallery_0..2.jpg`,
    # чтобы изменения исходников в `demo_images/gallery_*.jpg` отражались сразу.
    gallery_rows = [
        (0, (46, 58, 72)),
        (1, (58, 72, 88)),
        (2, (40, 50, 64)),
    ]
    for order, rgb in gallery_rows:
        dst = FRONT_DEMO / f"gallery_{order}.jpg"
        src = DEMO_IMAGES / f"gallery_{order}.jpg"
        if src.is_file() and src.stat().st_size > 500:
            dst.write_bytes(src.read_bytes())
        else:
            dst.write_bytes(jpeg_placeholder(rgb, w=1400, h=900))
